...
function headerType(v){const x=v.join(' ').toLowerCase();if(x.includes('sn')&&x.includes('nin')&&x.includes('sector')&&x.includes('facility name')&&x.includes('target per month'))return'facility';if(x.includes('sn')&&x.includes('nin')&&x.includes('sector')&&x.includes('no of facility')&&x.includes('target per month'))return'sector';return''}
function collect(a,start,end,type){const out=[];for(let i=start+1;i<(end<0?a.length:end);i++){const v=a[i];if(/^(total|योग|कुल)$/i.test(v[0]))break;if(/^\d+$/.test(v[0])){if(type==='facility'&&v[1]&&v[2]&&v[3])out.push(v);if(type==='sector'&&v[2]&&v[3])out.push(v)}}return out}
...
function render(dt){css();const a=getRows(dt);let fh=-1,sh=-1;for(let i=0;i<a.length;i++){const z=headerType(a[i]);if(z==='facility'&&fh<0)fh=i;else if(z==='sector'&&sh<0)sh=i}const facilities=fh>=0?collect(a,fh,sh,'facility'):[];const sectors=sh>=0?collect(a,sh,-1,'sector'):[];const rp=document.getElementById('reportPage');if(!rp)return;rp.querySelectorAll('.report-header,.status,.table-box,.report-actions,.footer').forEach(e=>e.style.display='none');let q=document.getElementById('wellnessActivityModule');if(!q){q=document.createElement('div');q.id='wellnessActivityModule';rp.appendChild(q)}let h='<div class="wellness-title">'+TITLE+'</div>';if(!facilities.length&&!sectors.length)h+='<div class="wellness-empty">Wellness Activity data नहीं मिला। Google Sheet की पहली 7 columns में header check करें।</div>';else{if(facilities.length)h+=facilityTable(facilities);if(sectors.length)h+=sectorTable(sectors)}q.innerHTML=h}
function load(){if(!window.google?.visualization?.Query){setTimeout(load,300);return}const q=new google.visualization.Query('https://docs.google.com/spreadsheets/d/'+SID+'/gviz/tq?gid='+GID+'&headers=0');q.setQuery('select A,B,C,D,E,F,G');q.send(r=>{if(r.isError()){const qx=document.getElementById('wellnessActivityModule');if(qx)qx.innerHTML='<div class="wellness-empty">Google Sheet data load error: '+esc(r.getMessage())+'</div>';return}render(r.getDataTable())})}
