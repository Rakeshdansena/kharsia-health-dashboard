/* Ayushman Shivir FINAL - 9 columns + calculated Total rows + dynamic Sheet heading */
(function(){'use strict';
...
function sheetTitle(all){
  ...
  const first=all[0].filter(Boolean);
  const candidate=first.find(x=>/ayushman|shivir|fy\s*20|till|upto|update|date|august|july|september|october|november|december|january|february|march|april|may|june/i.test(x));
  return candidate||first.find(Boolean)||FALLBACK_TITLE;
}
...
function makeTotal(rows,isSector){
  ...
  if(isSector){const facilities=valid.reduce((a,r)=>a+num(r[3]),0);return ['Total','','Total',String(facilities),String(target),String(rep),pct,String(foot),String(avg)];}
...
function box(title,heads,data,isSector){
  ...
  const w=document.createElement('div');w.className='shivir-wrap';const t=document.createElement('table');t.className='shivir-table';
  const hr=document.createElement('tr');heads.slice(0,COLS).forEach(x=>{const e=document.createElement('th');e.textContent=x;hr.appendChild(e)});const thead=document.createElement('thead');thead.appendChild(hr);t.appendChild(thead);
  const tb=document.createElement('tbody');data.filter(r=>!isTotal(r)).forEach(r=>{const tr=document.createElement('tr');for(let i=0;i<COLS;i++){const td=document.createElement('td');td.textContent=v(r[i]);if(i===6)td.className='shivir-pct';tr.appendChild(td)}tb.appendChild(tr)});
