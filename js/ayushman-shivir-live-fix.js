/* Ayushman Shivir — fixed renderer
   Reads the existing Shivir sheet directly and never uses the generic 27-column renderer. */
(function(){
  'use strict';
  ...
  function isData(r){return /^\d+$/.test(t(r[0]));}
  function isTotal(r){return t(r[0]).toLowerCase()==='total';}
  ...
  function makeSection(rows,title,sectorWise){
    ...
    const headers=sectorWise
      ?['Sn','NIN ID','Sector','No. of Facility','Target Upto 18 August 2026','Shivir Reporting','%','Total Footfall','Avg Footfall/Shivir','Total Footfall','Avg Footfall']
      :['Sn','NIN ID','Sector','Facility Name','Target Upto 18 August 2026','Shivir Reporting','%','Total Footfall','Avg Footfall/Shivir','Total Footfall','Avg Footfall'];
    ...
    rows.forEach(r=>{const tr=document.createElement('tr');r.slice(0,11).forEach((v,i)=>{const td=cell('td',v);if(i===6&&/%/.test(v))td.className='shivir-percent';tr.appendChild(td);});if(isTotal(r))tr.className='shivir-total';tbody.appendChild(tr);});
  ...
  function render(data){
    ...
    const titles=[];rows.forEach((r,i)=>{if(isTitle(r))titles.push({i,text:t(r.join(' '))});});
    const firstTitle=titles[0]?.text||DEFAULT_TITLE, secondTitle=titles[1]?.text||firstTitle;
    const first=[],second=[];let section=0;
    rows.forEach(r=>{
      if(isTitle(r)){section++;return;}
      if(section===1&&(isData(r)||isTotal(r)))first.push(r);
      else if(section>=2&&(isData(r)||isTotal(r)))second.push(r);
    });
    if(!first.length)return false;
    let old=pdf.querySelector('.shivir-container');if(old)old.remove();
    const container=document.createElement('div');container.className='shivir-container';
    container.appendChild(makeSection(first,firstTitle,false));
    if(second.length)container.appendChild(makeSection(second,secondTitle,true));
  ...
  function load(){
    ...
    if(!title||!/Ayushman Shivir/i.test(title.textContent||''))return;
    const pdf=document.getElementById('pdfArea');if(!pdf)return;
    if(pdf.dataset.shivirLoading==='1'||pdf.querySelector('.shivir-container'))return;
    ...
    if(window.google&&google.visualization){
      ...
      q.send(function(res){
        pdf.dataset.shivirLoading='0';
        if(res.isError()){if(window.setStatus)setStatus('Google Sheet से Ayushman Shivir data load नहीं हो पाया: '+res.getMessage(),'error');return;}
