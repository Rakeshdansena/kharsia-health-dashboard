...
function detect(r,current){const s=rt(r);if(/part\s*1|part-1|part–1|enrollment.*abha/i.test(s))return'enrollment';if(/part\s*2|part-2|part–2|\bhtn\b|hypertension|hypertensive/i.test(s))return'htn';if(/part\s*3|part-3|part–3|\bdm\b|diabetes|diabetic/i.test(s))return'dm';return current;}
function styles(){if(document.getElementById('ncd-module-style'))return;const s=document.createElement('style');s.id='ncd-module-style';s.textContent=`
